...
void exec(list<char> &main_list) {
    ...
    cout<<"df_MA_EX:"<<df_MA_EX<<"%";
    cout<<"df_MA_EX_value:"<<df_MA_EX_value<<"%";
    cout<<"pc:"<<pc<<"%";
...
int check_dep(bitset <M> d1, bitset <M> d2) {
    ...
    if (rs1_d1 == rd_d2 && rs1_d1 !=0) {
        if (opcode_d2 == "0000011") {
            df_MA_EX=1;return 0;
        } return 1;
    }
    if ((type_d1 == 'R' || type_d1 == 'S' || type_d1 == 'B') && (rs2_d1 == rd_d2) && (rs2_d1 != 0)) {
        if (opcode_d2 == "0000011") {
            if (rs1_d1 == rs2_d1) {
                df_MA_EX=3;return 0;
            }else {
...
void execute(){
    ...
    //df 
    if (df_MA_EX == 3) {
        P_DE_EX.op1 = df_MA_EX_value;
        P_DE_EX.op2 = df_MA_EX_value;
        cout<<"\nDATA VALUE FORWARDED FROM MA TO EX (MA_EX df) both op1 and op2 take forwarded value\n";
        df_MA_EX=0;
        df_MA_EX_value=0;
    } else if (df_MA_EX == 2) {
        P_DE_EX.op2 = df_MA_EX_value;
        cout<<"\nDATA VALUE FORWARDED FROM MA TO EX (MA_EX df) op2 take forwarded value\n";
        df_MA_EX=0;
        df_MA_EX_value=0;
    } else if (df_MA_EX == 1) {
        P_DE_EX.op1 = df_MA_EX_value;
        cout<<"\nDATA VALUE FORWARDED FROM MA TO EX (MA_EX df) op1 take forwarded value\n";
        df_MA_EX=0;
        df_MA_EX_value=0;
    }
...
void memory(){
    ...
    if (!P_EX_MA.nop) {
        ...
        if(P_EX_MA.inst_type=='I' && P_EX_MA.opcode=="0000011") {
            ...
            //df
            df_MA_EX_value = P_EX_MA.alu_result;
