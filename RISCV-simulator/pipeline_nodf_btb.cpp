...
//btb
int branch_in_execute=0;
int fetch_btb_prediction=0;
int fetch_btb_prediction_value=0;
...
int branchPredictorVar=0;

int branchPredictor() {
    return branchPredictorVar;
}
...
void exec(list<char> &main_list) {
    ...
    cout<<"clock:"<<int(clk)<<"%";
    if (!fetch_btb_prediction) {
    if(branchTaken){
        if (branch_in_execute) {
            if (!branchPredictor()) {
                branchPredictorVar=1;
                control_dep=1;
                pc=branchPc;
                cout<<"Branch Taken and btb Prediction:NOT Taken\n";
            }    
            else {
                cout<<"Branch Taken and btb Prediction:Taken\n"; 
                control_dep=0;
                pc+=4;
            }
            branch_in_execute=0;
        ...
        if (branch_in_execute) {
            if (branchPredictor()) {
                branchPredictorVar=0;
                control_dep=1;
                pc+=4;
                cout<<"%Branch NOT Taken and btb Prediction:Taken\n";
            }    
            else {
                cout<<"%Branch NOT Taken and btb Prediction:NOT Taken\n";
                pc+=4;
                control_dep=0;
            } 
            branch_in_execute=0;    
        ...
        pc=fetch_btb_prediction_value;
        fetch_btb_prediction=0;
    }
    cout<<"%pc:"<<pc<<"%";
    ...
    cout<<"f_btb_p:"<<fetch_btb_prediction<<"%";
    cout<<"f_btb_p_value:"<<fetch_btb_prediction_value<<"%";
...
void fetch(){
    ...
    //btb
    if ((jal_dep==0 && branch_in_execute && branchPredictor() == branchTaken) || (jal_dep==0 && !branch_in_execute)) {
        string curr_opcode = find_opcode(fetchResult);
        char curr_inst_type = pair_of_instr_type_opcode[curr_opcode];
        if (curr_inst_type == 'B') {
            int curr_imm = find_immed('B');
            if (branchPredictor()) {
                fetch_btb_prediction=1;
                fetch_btb_prediction_value = pc + curr_imm;
                cout<<"\n---btb predicted branch taken---\n";
            } else {
                fetch_btb_prediction=1;
                fetch_btb_prediction_value = pc + 4;
                cout<<"\n---btb predicted branch Not taken---\n";  
            }
        } else {
            cout<<"\n---NO branch prediction (Current inst is not branch)---\n";
        }
    } else {
        cout<<"\n---NO branch prediction (DUE TO EXECUTE)---\n";
    }
};
